...
import com.kauveryhospital.fieldforce.UseronlyAccess.unplanned.viewModel.MainActivityViewModel;
import com.kauveryhospital.fieldforce.databinding.ActivityMainBinding;
import com.kauveryhospital.fieldforce.databinding.ActivityPlannedMainBinding;
import java.util.ArrayList;
import java.util.List;

public class PlannedMainActivity extends AppCompatActivity implements IMainActivityContract.View {
    private static final String TAG = PlannedMainActivity.class.getSimpleName();
    ...
    private final Observer<DataResponse<ContactResponse>> importContactsFromCPObserver = contactResponse ->
    {
        ...
        if (contactResponse.getState() == StateDefinition.State.SUCCESS) {
            ...
            setupLottieAnimation(lottieAnimationView, ERROR_ANIMATION);
            String errorMessage = (contactResponse.getErrorData().getErrorStatus() + contactResponse.getErrorData().getErrorMessage());
    ...
    private final Observer<BooleanResponse> excelGenerationObserver = booleanResponse -> {
        ...
        if (booleanResponse.getState() == StateDefinition.State.SUCCESS) {
            ...
            String errorMessage = (booleanResponse.getErrorData().getErrorStatus() + booleanResponse.getErrorData().getErrorMessage());
    ...
    private final Runnable importContactsRunnable = new Runnable() {
        ...
        public void run() {
            ...
            mViewModel.initiateImport(uname,pswd);
        }
    };
    /**
